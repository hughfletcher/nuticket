...
class ActionCreateJobTest extends TestCase
{
    ...
    public function testHandleBaseParams()
    {
        ...
        $attrs = ['ticket_id' => 123, 'user_id' => 456, 'type' => 'reply', 'body' => $this->faker->sentence];
        $ticket = factory(App\Ticket::class)->make(['id' => $attrs['ticket_id']]);

        $class = new ReflectionClass('App\Jobs\ActionCreateJob');
        $job = $class->newInstanceArgs($attrs);

        $this->ticket->shouldReceive('find')
            ->with($attrs['ticket_id'])
            ->andReturn($ticket);
    ...
    {
        $this->expectsEvents(App\Events\ActionCreatedEvent::class);
        $attrs = array_slice(factory(App\TicketAction::class)->make(['type' => 'transfer', 'transfer_id' => 45])->toArray(), 0, 5);
        $ticket = factory(App\Ticket::class)->make(['id' => $attrs['ticket_id']]);
